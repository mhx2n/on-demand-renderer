...
_PRIVATE_ONLY_CMDS = {
    ...
    "owner", "stats", "logs", "users", "setchannel", "forcejoin",
...
async def cmd_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    fj_enabled = (await db.get_setting("force_join_enabled", "on")).strip().lower()
    fj_state = "ON" if fj_enabled not in ("off","0","false","no","disabled") else "OFF"
    ...
    await send_md(update.effective_message,
        ...
        f"• Channel: <code>{escape_html(ch)}</code>  •  Force-join: <code>{fj_state}</code>\n"
...
async def cmd_forcejoin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Owner: turn the force-join message on or off without losing the channel."""
    if not await _owner_only(update): return
    cur = (await db.get_setting("force_join_enabled", "on")).strip().lower()
    if not context.args:
        await update.effective_message.reply_text(
            f"Force-join is currently: <b>{'ON' if cur not in ('off','0','false','no','disabled') else 'OFF'}</b>\n"
    ...
    if arg in ("on", "enable", "enabled", "1", "true", "yes"):
        await db.set_setting("force_join_enabled", "on")
        await update.effective_message.reply_text("✅ Force-join message: <b>ON</b>", parse_mode=ParseMode.HTML)
    elif arg in ("off", "disable", "disabled", "0", "false", "no"):
        await db.set_setting("force_join_enabled", "off")
        await update.effective_message.reply_text("🔕 Force-join message: <b>OFF</b>", parse_mode=ParseMode.HTML)
    else:
        await update.effective_message.reply_text("Usage: /forcejoin on | off")
