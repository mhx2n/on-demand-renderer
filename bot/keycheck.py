...
def _detect(key: str) -> str:
    ...
    # Z.AI (formerly Zhipu) GLM keys: sk-zy-...
    if k.startswith("sk-zy-") or k.startswith("sk-zai-"): return "zai"
...
async def _zai(session, key):
    # Z.AI (formerly Zhipu / GLM) — OpenAI-compatible.
    # Global SaaS endpoint: api.z.ai. China endpoint: open.bigmodel.cn.
    bases = [
        ("https://api.z.ai/api/paas/v4", "Z.AI"),
        ("https://open.bigmodel.cn/api/paas/v4", "Z.AI (BigModel)"),
    ]
    last = None
    for base, label in bases:
        s, data, _ = await _fetch_json(session, "GET", f"{base}/models",
                                       headers={"Authorization": f"Bearer {key}"})
        if s == 200:
            ids = [m.get("id") for m in (data.get("data", []) if isinstance(data, dict) else []) if m.get("id")]
            if not ids:
                ids = ["glm-4.6", "glm-4.5", "glm-4.5-air", "glm-4-flash"]
            return {"provider": label, "kind": "zai", "valid": True, "models": ids[:80],
                    "limits": {"base_url": base}}
        last = {"provider": label, "kind": "zai", "valid": False, "status": s, "error": data}
    return last or {"provider": "Z.AI", "kind": "zai", "valid": False, "error": "no endpoint reachable"}
...
async def _probe_all(session, key, order=None):
    ...
    for name in order:
        ...
        if res.get("valid"):
            return res
        last = res
    return last or {"provider": "Unknown", "valid": False,
                    "error": "Unsupported key format — no known provider accepted this key. "
                             "Use /addprovider to register a custom OpenAI-compatible base URL."}
